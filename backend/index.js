...
app.get("/api/notifications", authenticate, (req, res) => {
  ...
  const notifications = unreadNotifications.map((contact) => ({
    id: contact.id,
    title: (
      contact.replies?.[contact.replies.length - 1]?.text ||
      contact.message ||
      ""
    ).slice(0, 60),
    ...
    lastReplyAt:
      contact.replies?.[contact.replies.length - 1]?.createdAt ||
      contact.updatedAt ||
      contact.createdAt,
  }));
...
app.post("/api/notifications/read/:id", authenticate, (req, res) => {
  ...
  const idx = contacts.findIndex(
    (contact) =>
      contact.id === id && (contact.email || "").toLowerCase() === userEmail,
  );
  if (idx === -1) {
